...
def transtofeature(countpositive,countnege,lamudamax,n):
    ...
    for i in range(countnege):
        print(i)
        linene=readfile('E:/Graduate/DNA-Binding/data/rank-pssm/newDNArset-negative/'+str(i)+'.pssm')
    ...
    np.savetxt('E:/Graduate/DNA-Binding/data/feature/pdb1075/'+str(n)+'a_'+str(lamudamax)+'_'+str(countnege+countpositive)+'.csv', feature, delimiter=',')
    label=[]
    label.extend(labelnpositive)
    label.extend(labelnegtive)

    np.savetxt('E:/Graduate/DNA-Binding/data/feature/pdb1075/'+str(n)+'b_'+str(lamudamax)+'_'+str(countnege+countpositive)+'.csv', label, delimiter=',')
#transtofeature()
import sys
